...
path_outdir <- "processed_data/MD1"
...
# Read cell metadata
df_meta <- read_csv(path_cell_metadata, col_names=c("cell","id","type"))

# Check input
head(df_meta)

# Add cell metadata to the Seurat object
pdac@meta.data <- cbind(pdac@meta.data, df_meta[, c("id", "type")])

# Yes, you can filter genes (features) here if you want

# Define sample_id
sample_ids <- unique(df_meta$id)
...
df_pdac_umap <-
  pdac_umap[["umap"]]@"cell.embeddings" %>%
    as.data.frame() %>%
    rownames_to_column("cell") %>%
    as_tibble %>%
    inner_join(df_meta, key = "cell") %>%
    mutate(grade = if_else(id %in% P_low_grade, "low", "high"))

ggplot(df_pdac_umap, aes(x = UMAP_1, y = UMAP_2)) +
