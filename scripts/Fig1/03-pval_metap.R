...
df <- read_csv("results/Fig1_ICGC/LR_Pval_HR.csv")
...
df_meta_pval <- df_wider %>%
  ...
  mutate(adj_Pval=meta_pval_storey) %>%
  inner_join(df) %>%
  select(LR, HR, Pval, meta_Pval, adj_Pval)
...
write_csv(df_result, "results/Fig1_ICGC/LR_selected.csv")
