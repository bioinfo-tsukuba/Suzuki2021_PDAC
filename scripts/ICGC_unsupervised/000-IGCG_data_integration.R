...
FeaturePlot(seurat_combined, features = df_seurat_markers$gene, 
...
# Violin plot
VlnPlot(seurat_combined, features = df_seurat_markers$gene,
            group.by = 'seurat_clusters') + theme(aspect.ratio=1)
...
for(i in 1:nrow(df_survival)){
  ...
  FeaturePlot(seurat_combined, reduction = 'umap', features = c(l, r),
...
for(i in 1:nrow(df_survival)){
  l <- df_survival[i,]$ligand
  r <- df_survival[i,]$receptor
  VlnPlot(seurat_combined, features = c(l, r),
            group.by = 'seurat_clusters') + theme(aspect.ratio=1)
