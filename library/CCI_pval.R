...
df_pval <-
  ...
  group_by(CCI, exp_bin) %>%
  mutate(median_time = median(time)) %>%
  select(CCI, time, status, exp_bin, median_time) %>%
  distinct() %>%
  # Report P-value
  nest(data = !c(CCI)) %>%
  ...
  arrange(pval) %>%
  unnest(data) %>%
  select(CCI, pval, hr, exp_bin, median_time) %>%
  distinct() %>%
  pivot_wider(c(CCI, pval, hr), names_from = "exp_bin", names_prefix = "median_day_", values_from = "median_time") %>%
